...
export class Agent {
  constructor(
    private opts: { brains: Brains; journal: Journal; tenantId: string; innerMe: string },
  ) {}

  async handleMessage(text: string): Promise<string> {
    const { brains, journal, tenantId, innerMe } = this.opts;
    ...
    try {
      for (let turn = 0; turn < MAX_TURNS; turn++) {
        const res = await brains.complete({ tier: "strong", system: innerMe, messages, tools });
